...
IOStatus Zone::Read(char *data, uint32_t size, uint64_t offset, bool direct) {
  ...
  // ZENFS_ZONE_READ_LATENCY,
  //  Env::Default());
  // zbd_->GetMetrics()->ReportThroughput(ZENFS_ZONE_READ_THROUGHPUT, size);
  /*  int ret = 0;
   int left = size;
   int r = -1;
...
void Zone::Print() {
  ...
  printf(KBOLD "[ZoneID:%5lu]start:" KRESET COLOR_BOLD_BLUE
         ...
         GetZoneNr(), start_, wp_, capacity_, max_capacity_,
         used_capacity_.load(), zone->cond, zbd_zone_cond_str(zone, false));
}
...
/* open and return a zone from offset in logical page id */
Zone *ZonedBlockDevice::GetIOZoneFromOffset(uint64_t offset) {
...
// Info(logger_, "New Zoned Block Device: %s",
// zbd_be_->GetFilename().c_str());
//}
...
void ZonedBlockDevice::LogZoneStats() {
  ...
  /*   Info(logger_,
         ...
         "%lu %lu %lu %lu %ld %ld\n",
         time(NULL) - start_time_, used_capacity / MB, reclaimable_capacity /
     MB, 100 * reclaimable_capacity / reclaimables_max_capacity, active,
...
//   return NoSpace("Out of metadata zones");
// }
...
//   if (finish_threshold_ == 0) return OK();
